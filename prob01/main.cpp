...
int main()
{
  ...
  // the value of the character isnt alphabetic because it no longer correspionds to that character
  std::cout << std::endl; // blank line to separate output from the different exercises
  ...
  std::cout << ", after adding 1: " << std::dec << my_short2
    ...
    // the reason my short became negative is because it overflowed and went back to lowest possible value
  std::cout << "Int (" << sizeof(my_short2) << " bytes): "
    << std::dec << my_int2 << std::hex << " (0x" << my_int2 << ')';
  my_int2 = my_int2 + 1 ;
  std::cout << ", after adding 1: " << std::dec << my_int2
    << std::hex << " (0x" << my_int2 << ")\n";
    // int did not change into a negative because the data type can hold more more digits
  std::cout << std::endl; // blank line to separate output from the different exercises
